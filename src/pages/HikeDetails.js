...
export default function HikeDetails() {
  ...
  const handleFavoriteClick = async () => {
    ...
    try {
      const response = await postFavorite(selectedHike.id, user.id);
  
      if (!response.ok) {
        // If the response status is not ok, throw an error with the server's response message
        throw new Error(response.statusText);
      }
  
      console.log("Hike added to favorites");
    } catch (error) {
      console.error("Failed to update favorites:", error.message); // Log the error message from the server
    }
  };
